...
start_ganache() {
  node_modules/.bin/ganache-cli --networkId 1337 --chainId 1337 -l 20000000 --port "$ganache_port" --accounts 20 > /dev/null &
  ganache_pid=$!
...
run_test() {
    ...
    printf "\n\n /************** COPY GAS Log "$testFileName" **********************/ \n\n" 

    cp "$path"/eth-gas-reporter.log "$path"/eth-gas-reporter-logs/"$timestamp"-"$testFileName".log
    cat "$path"/eth-gas-reporter.log >> "$path"/eth-gas-reporter-logs/"$timestamp"-0all.log


    printf "\n\n /************** END COPY GAS LOG "$testFileName" **********************/ \n\n" 
