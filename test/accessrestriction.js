...
contract('AccessRestriction', (accounts) => {
    ...
    it("should add admin", async () => {
        ...
        let tx = await arInstance.grantRole(DEFAULT_ADMIN_ROLE, adminAccount, { from: deployerAccount });

        truffleAssert.eventEmitted(tx, 'RoleGranted', (ev) => {
            return ev.account.toString() === adminAccount && ev.role === DEFAULT_ADMIN_ROLE;
        });
